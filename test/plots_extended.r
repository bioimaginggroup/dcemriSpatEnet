...
require("fields")
  ...
  for(k in 1:K)
  {
     error[,,k] <- Response[,,k] - spatFit_list[[k]]$reducedFit
  }
  ...
  name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/sse_k_spatial_extended_1.pdf"
  ...
  name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/msse_spatial_extended_1.pdf"
  pdf(name, width = 4, height = 4)
  par(mfrow=c(1,1), mar=c(1, 1, 1, 1), cex=1)

      sse_matrix <- t(matrix(msse, nrow=J))
      sse_matrix[75,75] <- 0.1
      z_range <- c(0,0.25)
...
for(k in 1:K)
{
  for(n in 1:N)
  {
  	 ...
  	 number[n,k] <- length(which(which(spatFit_list[[k]]$reducedTheta[n,]!=0)!=1))

  	 vp_array[n,k] <- length(which(which(spatFit_list[[k]]$reducedTheta[n,]!=0)==1))
...
for(n in 1:N)
{
    	 mnumber[n] <-  sum(number[n,])/K
    	 mvp_array[n] <-  sum(vp_array[n,])/K
}


name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/mq_estimates_extended_1.pdf"
pdf(name, width = 4, height = 4)
par(mfrow=c(1,1), mar=c(1, 1, 1, 1), cex=1)
...
name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/mvp_estimates_extended_1.pdf"
pdf(name, width = 4, height = 4)
par(mfrow=c(1,1), mar=c(1, 1, 1, 1), cex=1)
...
name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/q_legend_extended_1.pdf"
...
name <- "~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/vp_legend_extended_1.pdf"
pdf(name, width = 1, height = 4)
par(mfrow=c(1,1), mar=c(0.1, 0.1, 0.1, 0.1),  cex=1.4)
