...
K <- 3  # number of simulated images
...
Response <- array(NA, c(I*J,T,K))
Fit <- array(NA, c(I*J,T,K))
Theta <- array(NA, c(I*J,Q,K))   # 2D array of Parameter values per Pixel
...
#  for(j in 1:J)
#  {
#     summed_curve <- summed_curve +  CONC_SIM_NOISE[i,j,,1]
#  }
#}
...
#}
#
#save(MeanTheta,file="~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/sim_MeanTheta_startingvalue_extended_1.Rdata")

load("~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/sim_MeanTheta_startingvalue_extended_1.Rdata")


## ------------------------------------------------------------------------------
## Voxelwise Fit
#Theta <- array(NA, c(I*J,Q,K))
#Fit <- array(NA, c(I*J,T,K))
#Theta2 <- array(0, c(I*J,Q,K))
#Fit2 <- array(NA, c(I*J,T,K)) 
##Selected_array <- vector("list", N,K)
#
#
#for(k in 1:K)
#{
...
#        #print(single_index)
#        
#        #test <- nnenet2(scaledX, Response[indx(single_index,I,J),,k], lambda=10^(-10), 1, firstpen=TRUE)
#        
#        test <- nnenet2(scaledX, Response[indx(single_index,I,J),,k], lambda=10^(-10), 1.5, firstpen=TRUE)
#        
#        Theta[indx(single_index,I,J),,k] <- test$beta
#        Fit[indx(single_index,I,J),,k] <- test$fit
#        Theta2[indx(single_index,I,J),test$selected,k] <- test$reducedBeta
#        Fit2[indx(single_index,I,J),,k] <- test$reducedFit
#        #Selected_array[[indx(single_index,I,J)],,k] <- test$selected
#  }
#}
#
#save(Response,scaledX,Theta,Fit,Theta2,Fit2,file="~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/voxelwise/sim_Theta_voxelwise_extended_3.Rdata")
#
...
for(k in 1:K)
{
  spatFit <- spatialFit(xyblack_list, xywhite_list, I=I, J=J, x=scaledX, Response=Response[,,k], time=time,
              Theta=MeanTheta, lambda=lambda2, sv=sv2, firstpen=TRUE, centersOnly=TRUE)
              
  spatFit_list[[k]] <- spatFit   
}         

save(Response,scaledX,time,spatFit_list,sv2,lambda2,file="~/Dateien/Projekte/Jan-Gertheis/Code_spatEnet/results/spatial/spatFit_extended_4.Rdata")
